...
class WanTransformer3DModel(
    ...
    def forward(
        ...
        hidden_states = hidden_states.flatten(2).transpose(1, 2)  # [B, C, F, H, W] -> [B, F*H*W, C]
...
from unet import spatially_variable_gaussian_blur2d


class MyWanTransformer3DModel(WanTransformer3DModel):
    def inject(self, kv_weight: Optional[torch.Tensor] = None, query_blur_sigma: Optional[torch.Tensor] = None):
        for block_idx, block in enumerate(self.blocks):
            block.attn1.processor = self.my_self_attention(block_idx, kv_weight, query_blur_sigma)

    record_blocks: list[int] = []
    latent_shape_: tuple[int, int, int] = ()
    record_attention_: bool | None = None
    record_query_ = []
    record_key_ = []
    record_value_ = []

    def my_self_attention(self, block_idx: int, kv_weight: Optional[torch.Tensor] = None, query_blur_sigma: Optional[torch.Tensor] = None):
        f, h, w = self.latent_shape_
        p_t, p_h, p_w = self.config.patch_size
        ppf, pph, ppw = f // p_t, h // p_h, w // p_w
        assert p_t == 1, "only support t_patch=1"
        ...
        def processor(
            ...
            hidden_states_img = None
            if encoder_hidden_states_img is not None:
                raise NotImplementedError("This custom attention should only be used in self attention.")
                key_img, value_img = _get_added_kv_projections(attn, encoder_hidden_states_img)
                key_img = attn.norm_added_k(key_img)
            ...
            if self.record_attention_ and block_idx in self.record_blocks:
                self.record_query_.append(rearrange(query, "() (f pph ppw) heads dim -> f heads (pph ppw) dim", f=ppf, pph=pph, ppw=ppw))
                self.record_key_.append(rearrange(key, "() (f pph ppw) heads dim -> f heads (pph ppw) dim", f=ppf, pph=pph, ppw=ppw))
            ...
            if query_blur_sigma is not None:
                chans = query.shape[-1]
                query_reshaped = rearrange(query, "b (ppf pph ppw) heads c -> (b ppf) (heads c) pph ppw", ppf=ppf, pph=pph, ppw=ppw)
                ...
                query = rearrange(query_blurred, "(b ppf) (heads c) pph ppw -> b (ppf pph ppw) heads c", ppf=ppf, pph=pph, ppw=ppw, c=chans)
    ...
    def forward(
        ...
        self.latent_shape_ = (num_frames, height, width)
