...
class DepthCompletion(torch.nn.Module):
    ...
    def knn_aligns(
        ...
        # NOTE: The CUDA KD-tree (cuda_kdtree.cu) has leaf_max_size=64.
        # When n_tree < 64, prealloc = n_tree / 64 * 16 = 0 (integer division),
        # causing zero-size GPU buffers and cudaErrorInvalidValue.
        # Fall back to brute-force torch.cdist for this edge case.
        _KDTREE_LEAF_MAX_SIZE = 64
        if x.shape[0] >= _KDTREE_LEAF_MAX_SIZE:
            import vipe_ext as _C
            _, inds = _C.utils_ext.nearest_neighbours(y, x, K)
            knn_indices = inds.view(-1, K)  # [M, K]
        else:
            # Brute-force fallback: compute all pairwise distances [M, N] and pick top-K.
            dists_matrix = torch.cdist(y.float(), x.float())  # [M, N]
            knn_indices = dists_matrix.topk(K, dim=1, largest=False).indices  # [M, K]
