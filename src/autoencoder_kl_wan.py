"""
Memory-efficient version of WanVAE.
...
class WanResidualBlock(nn.Module):
    ...
    def forward(self, x, feat_cache=None, feat_idx=[0]):
        ...
        if feat_cache is not None:
            ...
            x = self.conv2(x, feat_cache[idx].to(x.device) if feat_cache[idx] is not None else None)
            feat_cache[idx] = cache_x.cpu()
            feat_idx[0] += 1
        else:
            x = self.conv2(x)
    ...
    def forward(self, x, feat_cache=None, feat_idx=[0]):
        if feat_cache is not None:
            ...
            if cache_x.shape[2] < 2 and feat_cache[idx] is not None:
                # cache last frame of last two chunk
                cache_x = torch.cat([feat_cache[idx][:, :, -1, :, :].unsqueeze(2).to(cache_x.device), cache_x], dim=2)
            x = self.conv_in(x, feat_cache[idx].to(x.device) if feat_cache[idx] is not None else None)
            feat_cache[idx] = cache_x.cpu()
            feat_idx[0] += 1
        else:
            x = self.conv_in(x)
        ...
        if feat_cache is not None:
            ...
            if cache_x.shape[2] < 2 and feat_cache[idx] is not None:
                # cache last frame of last two chunk
                cache_x = torch.cat([feat_cache[idx][:, :, -1, :, :].unsqueeze(2).to(cache_x.device), cache_x], dim=2)
            x = self.conv_out(x, feat_cache[idx].to(x.device) if feat_cache[idx] is not None else None)
            feat_cache[idx] = cache_x.cpu()
            feat_idx[0] += 1
        else:
            x = self.conv_out(x)
...
class AutoencoderKLWan(ModelMixin, AutoencoderMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def _decode(self, z: torch.Tensor, return_dict: bool = True):
        ...
        # Run first chunk
        self._conv_idx = [0]
        out_chunk_0 = self.decoder(
            x[:, :, 0:1, :, :], feat_cache=self._feat_map, feat_idx=self._conv_idx, first_chunk=True
        )

        if num_frame == 1:
            out = out_chunk_0
        else:
            # Run second chunk to determine the shape of the rest
            self._conv_idx = [0]
            out_chunk_1 = self.decoder(x[:, :, 1:2, :, :], feat_cache=self._feat_map, feat_idx=self._conv_idx)

            b, c, t_chunk_0, h, w = out_chunk_0.shape
            _, _, t_chunk_rest, _, _ = out_chunk_1.shape

            total_t = t_chunk_0 + t_chunk_rest * (num_frame - 1)
            out = torch.empty(
                (b, c, total_t, h, w),
                dtype=out_chunk_0.dtype,
                device=out_chunk_0.device
            )

            out[:, :, :t_chunk_0, :, :] = out_chunk_0
            out[:, :, t_chunk_0 : t_chunk_0 + t_chunk_rest, :, :] = out_chunk_1

            current_t = t_chunk_0 + t_chunk_rest

            for i in range(2, num_frame):
                self._conv_idx = [0]
                out_chunk = self.decoder(x[:, :, i : i + 1, :, :], feat_cache=self._feat_map, feat_idx=self._conv_idx)
                out[:, :, current_t : current_t + t_chunk_rest, :, :] = out_chunk
                current_t += t_chunk_rest

                if i % 10 == 0:
                    # print(f"Step {i}: {torch.cuda.memory_allocated() / 1024**3:.2f} GB allocated.")
                    pass
